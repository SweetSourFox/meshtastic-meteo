...
int CannedMessageModule::handleInputEvent(const InputEvent *event)
{
    ...
    case CANNED_MESSAGE_RUN_STATE_INACTIVE:
        ...
        // Printable char (ASCII) opens free text compose — unless another UI module owns the keyboard.
        if (event->kbchar >= 32 && event->kbchar <= 126) {
            auto uiModules = MeshModule::GetMeshModulesWithUIFrames(0);
            for (MeshModule *mod : uiModules) {
                if (mod && mod != this && mod->interceptingKeyboardInput()) {
                    // #region agent log
                    LOG_INFO("METEO_DBG {\"sessionId\":\"d53c55\",\"hypothesisId\":\"H-canned\",\"location\":\"CannedMessageModule.cpp:INACTIVE\",\"message\":\"defer-to-module\",\"data\":{\"ch\":%u}}",
                             unsigned(event->kbchar));
                    // #endregion
                    return 0;
                }
            }
            // #region agent log
            LOG_INFO("METEO_DBG {\"sessionId\":\"d53c55\",\"hypothesisId\":\"H-canned\",\"location\":\"CannedMessageModule.cpp:INACTIVE\",\"message\":\"open-freetext\",\"data\":{\"ch\":%u}}",
                     unsigned(event->kbchar));
            // #endregion
            updateState(CANNED_MESSAGE_RUN_STATE_FREETEXT, true);
            UIFrameEvent e;
